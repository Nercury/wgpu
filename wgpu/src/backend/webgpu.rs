...
use crate::{context::{downcast_ref, QueueWriteBuffer}, CompilationInfo, CompilationMessageType, SurfaceTargetUnsafe, UncapturedErrorHandler};
...
impl crate::context::Context for ContextWebGpu {
    ...
    fn device_try_create_shader_module(
        ...
        desc: crate::ShaderModuleDescriptor<'_>,
        _shader_bound_checks: wgt::ShaderBoundChecks,
    ) -> Result<Self::ShaderModuleData, String> {
        let shader_module_result = match desc.source {
            #[cfg(feature = "spirv")]
            crate::ShaderSource::SpirV(ref spv) => {
                use naga::front;
        ...
        let (mut descriptor, compilation_info): (webgpu_sys::GpuShaderModuleDescriptor, WebShaderCompilationInfo) = match shader_module_result {
            Ok(v) => v,
            Err(ci) => {
